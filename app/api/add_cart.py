...
def add_cart(product_id:int,quantity:int=None,db:Session=Depends(get_db),user_data=Depends(get_current_user)):
    ...
    get_cart=db.query(Cart).filter(Cart.product_id==product_id,Cart.user_id==user_data.id).first()
    if get_cart:
